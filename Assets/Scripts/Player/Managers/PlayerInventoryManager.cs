...
namespace FN
{
    public class PlayerInventoryManager : CharacterInventoryManager
    {
        ...
        {
            // Check if there are any consumable items in the array
            if (consumableSlots.Length > 0)
            {
                // Increment the current index and wrap around to 0 if it exceeds the array length
                currentConsumableIndex = (currentConsumableIndex + 1) % consumableSlots.Length;

                // Get the consumable item at the current index
                currentConsumable = consumableSlots[currentConsumableIndex];

                // TODO: Perform any additional logic needed when the consumable item changes
            }
        }
    }
}